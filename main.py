...
logger = logging.getLogger("GuardPanel")
...
async def load_initial_data():
    ...
    if not LINKS:
        ...
        await db_execute(
            ...
            (default_uuid, "Your Subscription", 0, 0, now, None),
        )
...
app = FastAPI(title="Guard Panel", lifespan=lifespan, docs_url=None, redoc_url=None)
...
SESSION_COOKIE = "guard_panel_session"
...
def generate_vless_link(uid: str, remark: str = "Guard", address: str = None, extra: dict = None) -> str:
...
async def root():
    return {"service": "Guard Panel", "version": "1.1.0", "status": "active", "domain": get_domain()}
...
async def notify_telegram_login(ip: str, ua: str):
    ...
    if lang == 'fa':
        default_login = f"🔐 ورود Guard\n🌐 IP: {ip}\n🤖 UA: {ua}\n📅 {now_str}"
    else:
        default_login = f"🔐 Guard Panel login\n🌐 IP: {ip}\n🤖 UA: {ua}\n📅 {now_str}"
    ...
    msg += f'\n\n<a href="{panel_url}">Open Guard Panel</a>'
    url = f"https://api.telegram.org/bot{token_row['value']}/sendMessage"
    try:
...
async def create_link(request: Request, _=Depends(require_auth)):
    body = await request.json()
    label = (body.get("label") or "Your Subscription").strip()[:60]
    ...
    return {
        ...
        "vless_link": generate_vless_link(uid, remark=f"Guard-{label}", extra=extra),
    }
...
async def list_links(_=Depends(require_auth)):
    ...
    for row in items:
        ...
        result.append({
            ...
            "color": row.get("color", "#e8e8e8"),
            "flag": row.get("flag", ""),
            "fragment": row.get("fragment", ""),
            "current_connections": await count_connections_for_link(uid),
            "vless_link": generate_vless_link(uid, remark=f"Guard-{row['label']}", extra=extra),
        })
    return {"links": result}
    ...
    async with LINKS_LOCK:
        for uid in uids:
            ...
            if action == "activate":
                ...
                if link.get("label") == "Your Subscription":
                    continue
...
async def regenerate_uuid(uid: str, _=Depends(require_auth)):
    async with LINKS_LOCK:
        ...
        if LINKS[uid].get("label") == "Your Subscription":
...
async def toggle_link(uid: str, request: Request, _=Depends(require_auth)):
    ...
    async with LINKS_LOCK:
        ...
        if not link:
            raise HTTPException(status_code=404, detail="link not found")
        if link.get("label") == "Your Subscription":
            if "label" in body and body["label"].strip() != "Your Subscription":
...
async def delete_link(uid: str, _=Depends(require_auth)):
    async with LINKS_LOCK:
        link = LINKS.get(uid)
        if link and link.get("label") == "Your Subscription":
            raise HTTPException(status_code=400, detail="Default inbound (Your Subscription) cannot be deleted.")
    await db_execute("DELETE FROM links WHERE uid = ?", "DELETE FROM links WHERE uid = $1", (uid,))
    async with LINKS_LOCK:
        LINKS.pop(uid, None)
    await close_connections_for_link(uid)
...
.card{{background:rgba(20,20,20,0.9);border:1px solid rgba(255,255,255,0.15);border-radius:24px;padding:36px 24px;max-width:420px;width:100%;box-shadow:0 0 40px rgba(255,255,255,0.1);text-align:center;}}
h1{{color:#e8e8e8;font-size:1.8rem;margin-bottom:8px;font-weight:800;}}
...
.btn{{display:flex;align-items:center;justify-content:center;width:100%;padding:14px;background:linear-gradient(135deg,#f5f5f5,#c0c0c0);color:#000;font-weight:800;border-radius:12px;text-decoration:none;transition:all 0.2s;margin-bottom:12px;border:none;cursor:pointer;font-family:inherit;font-size:1rem;}}
.btn:hover{{filter:brightness(1.1);box-shadow:0 0 20px rgba(255,255,255,0.3);}}
.btn-outline{{background:transparent;color:#e8e8e8;border:2px solid rgba(255,255,255,0.3);}}
.btn-outline:hover{{background:rgba(255,255,255,0.1);box-shadow:none;}}
#toast{{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#e8e8e8;color:#000;padding:10px 20px;border-radius:30px;font-weight:700;opacity:0;transition:opacity 0.3s;pointer-events:none;}}
</style>
</head>
<body>
<div class="card">
...
def generate_subscription_content(link: dict, uid: str, addresses: list, extra: dict = None, status: str = "active") -> str:
    ...
    server_node = generate_vless_link(uid, remark=f"{flag_emoji}Your Subscription" if flag_emoji else "Your Subscription", extra=extra)
    links = [status_node, server_node]
    for i, addr in enumerate(addresses):
        links.append(generate_vless_link(uid, remark=f"{flag_emoji}Guard-{link['label']}-IP{i+1}" if flag_emoji else f"Guard-{link['label']}-IP{i+1}", address=addr, extra=extra))
    return "\n".join(links)
...
*{margin:0;padding:0;box-sizing:border-box}
:root{
  --primary:#e8e8e8; --primary-dim:rgba(232,232,232,0.12);
  --bg:#0a0a0a; --bg2:#121212; --bg3:#1a1a1a;
  --surface:rgba(20,20,20,0.85); --surface2:rgba(30,30,30,0.9); --surface3:rgba(40,40,40,0.8);
  --border:rgba(255,255,255,0.08); --border2:rgba(255,255,255,0.2);
  --text:#e0e0e0; --text2:#a0a0a0; --text3:#707070;
  --green:#4ade80; --red:#f87171; --yellow:#fbbf24;
  --header-h:60px; --footer-h:50px;
}
body.light-mode {
  --primary:#111111; --primary-dim:rgba(0,0,0,0.12);
  --bg:#f8f8f8; --bg2:#ffffff; --bg3:#eeeeee;
  --surface:rgba(255,255,255,0.9); --surface2:rgba(255,255,255,0.95); --surface3:rgba(245,245,245,0.95);
...
.btn-primary{background:linear-gradient(135deg,#f5f5f5,#c0c0c0);color:#000;box-shadow:0 0 16px rgba(255,255,255,0.3)}
.btn-primary:hover{filter:brightness(1.2);box-shadow:0 0 24px rgba(255,255,255,0.35)}
...
.glass-btn-group {display: flex;flex-wrap: wrap;gap: 6px;background: rgba(255, 255, 255, 0.04);border: 1px solid rgba(255, 255, 255, 0.1);padding: 5px;border-radius: 14px;backdrop-filter: blur(16px);-webkit-backdrop-filter: blur(16px);box-shadow: inset 0 1px 0 rgba(255,255,255,0.05);}
.glass-btn {flex: 1;min-width: 80px;background: transparent;border: 1px solid transparent;color: var(--text3);padding: 9px 14px;border-radius: 10px;cursor: pointer;font-weight: 600;font-family: inherit;font-size: 0.85rem;transition: all 0.25s cubic-bezier(0.4,0,0.2,1);}
.glass-btn.active {background: rgba(255,255,255,0.15);color: var(--primary) !important;border-color: rgba(255,255,255,0.2);box-shadow: 0 4px 16px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.12);}
...
.status-glass-card.active {background: rgba(255, 255, 255, 0.08);border: 1px solid rgba(255, 255, 255, 0.22);color: var(--primary);box-shadow: 0 0 20px rgba(255,255,255,0.06); position:relative;}
.status-glass-card.active::after{content:'';position:absolute;top:0;left:10%;right:10%;height:2px;background:linear-gradient(90deg,transparent,var(--primary),transparent);border-radius:2px;}
    ...
    <div style="background:var(--surface2);border:1px solid var(--border2);border-radius:28px;padding:48px 40px;width:100%;max-width:400px;box-shadow:0 0 40px var(--primary-dim);backdrop-filter:blur(20px);">
      <div style="text-align:center;margin-bottom:32px;">
        <svg width="100%" viewBox="0 0 180 80" height="100%">
          ...
          <text x="90" y="58" font-family="'Orbitron',sans-serif" font-size="40" font-weight="900" fill="var(--primary)" text-anchor="middle">Guard</text>
        </svg>
        <div style="font-family:'Orbitron',sans-serif;font-size:1.5rem;font-weight:900;color:var(--primary);margin-top:12px;display:flex;align-items:center;justify-content:center;gap:8px;">
          Guard Panel <span style="font-size:0.8rem; font-family:'Inter'; color:var(--bg); background:var(--primary); padding:2px 6px; border-radius:4px;">V 1.1.0</span>
        </div>
      ...
      <div style="margin-top:20px; text-align:center; display:flex; justify-content:center; gap:20px;">
        <a href="#" target="_blank" style="color:var(--text3); text-decoration:none; font-size:0.9rem;" title="GitHub">🐙 GitHub</a>
        <a href="#" target="_blank" style="color:var(--text3); text-decoration:none; font-size:0.9rem;" title="Telegram">📨 Telegram</a>
      </div>
    </div>
  </div>
</div>
  ...
  <header class="header">
    <div class="header-inner">
      <div style="display:flex;align-items:center;gap:16px;">
        <span class="logo">Guard</span><span class="version-tag">v1.1.0</span>
...
192.168.1.0/24"></textarea></div>
    ...
    <section class="page" id="page-telegram">
      ...
      <div class="card">
        ...
        <div class="fg"><label class="fl">Custom Templates (EN)</label>
          <textarea class="fi" id="tg-templates-en" rows="4">{"quota_90":"⚠️ {label} ({uid}) used 90% of quota","login":"🔐 Guard Panel login\n🌐 IP: {ip}\n🤖 UA: {ua}\n📅 {time}","expiry":"⏰ {label} expired","error":"❌ Error on {label}: check logs"}</textarea>
        </div>
        <div class="fg"><label class="fl">Custom Templates (FA)</label>
          <textarea class="fi" id="tg-templates-fa" rows="4">{"quota_90":"⚠️ {label} ({uid}) ۹۰٪ کوتا","login":"🔐 ورود Guard\n🌐 IP: {ip}\n🤖 UA: {ua}\n📅 {time}","expiry":"⏰ {label} منقضی شد","error":"❌ خطا در {label}: بررسی شود"}</textarea>
        </div>
  ...
  <footer class="footer">
    <div class="footer-inner">
      ...
      <a href="#/Guard-Panel" target="_blank">Project Repo</a>
    </div>
  </footer>
</div>
...
<div class="mo" id="mo-add">
  <div class="mo-box">
    ...
    <div class="fg"><label class="fl" data-en="Name" data-fa="نام">Name</label><input class="fi" id="nl" placeholder="Your Subscription" maxlength="60"></div>
    ...
    <div class="fg"><label class="fl" data-en="Color" data-fa="رنگ">Color</label><input type="color" id="alink-color" value="#e8e8e8"></div>
...
<div class="mo" id="mo-edit">
  <div class="mo-box">
    ...
    <div class="fg"><label class="fl" data-en="Color" data-fa="رنگ">Color</label><input type="color" id="e-color" value="#e8e8e8"></div>
...
const footerTexts = {
  en: 'Secure Subscription Management Panel',
  fa: 'پنل مدیریت اشتراک امن'
};
...
async function createLink(){
  const label=$m('nl').value.trim()||'Your Subscription';
  ...
  const body={
    ...
    color:$m('alink-color')?.value||'#e8e8e8', flag: flagCode, fragment: fragment
  };
  try{
...
function showEditMo(uid){
  ...
  $m('e-color').value=l.color||'#e8e8e8';
  const flag = l.flag || '';
...
function dlQR(){const a=document.createElement('a');a.href=$m('qr-img').src;a.download='guard-qr.png';a.click();}
...
function initChart(){
  ...
  tChart=new Chart(ctx,{
    type:'bar',
    data:{labels:[],datasets:[{label:'MB',data:[],backgroundColor:'rgba(200,200,200,0.6)',borderColor:'#e8e8e8',borderWidth:1,barPercentage:0.7,categoryPercentage:0.9}]},
    options:{
      responsive:true, maintainAspectRatio:false,
      plugins:{legend:{display:false}},
      scales:{x:{ticks:{color:'rgba(255,255,255,0.3)',maxRotation:45}},y:{ticks:{color:'rgba(255,255,255,0.3)',callback:v=>v+' MB'},beginAtZero:true}}
    }
  });
  updChartColors();
}
function updChartColors(){if(!tChart)return;const col=theme==='light'?'#000':'rgba(255,255,255,0.25)';tChart.options.scales.x.ticks.color=col;tChart.options.scales.y.ticks.color=col;tChart.update();}
...
function updDoughnutChart(){if(!doughnutChart)return;const labels=[],data=[],colors=[];allLinks.filter(l=>l.used_bytes>0).forEach(l=>{labels.push(l.label);data.push(l.used_bytes);colors.push(l.color||'#e8e8e8');});doughnutChart.data.labels=labels;doughnutChart.data.datasets[0].data=data;doughnutChart.data.datasets[0].backgroundColor=colors;doughnutChart.update();}
...
async function exportLinks(){try{const r=await fetch('/api/export-links');const data=await r.json();const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='guard-links.json';a.click();}catch{toast('Export failed',true);}}
...
async function testTelegram(){const token=$m('tg-token').value.trim(),chat=$m('tg-chat-id').value.trim();if(!token||!chat){toast('Fill token and chat ID',true);return;}const tglang=$m('tg-lang-hidden').value;const msg = tglang==='fa'?'✅ پنل Guard متصل شد':'✅ Guard Panel is connected';try{const res=await fetch(`https://api.telegram.org/bot${token}/sendMessage`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chat,text:msg})});if(res.ok)toast('Test message sent!');else toast('Failed to send',true);}catch{toast('Error',true);}}
...
function previewTemplate() {
    ...
    try {
        ...
        const mockData = {
            label: "Guard_User", uid: "guard-7b8c-49ed-b45a",
        ...
        previewHTML += `⚠️ <i>Auto Appended:</i><br>Open Guard Panel (Link: https://${mockDomain}/panel)`;
        previewHTML += `</div>`;
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Guard Panel on port {port}")
    try:
        subprocess.run(
            [
